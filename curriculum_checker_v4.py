...
def run_checks(xlsx_path: str):
    ...
    if not hidden_name:
        issues.append({"severity": "ERROR", "sheet": "-", "row": "-", "message": "목록 시트를 찾지 못했습니다(시트명에 '숨김' 포함 필요)."})
    ...
    while True:
        ...
        if course_norm in hidden:
            issues.append({
                ...
                "sheet": hidden_name,
                "row": r,
                "message": f"목록에 중복 과목명이 있습니다: '{course_norm}' (기존 {hidden[course_norm]['row']}행, 추가 {r}행). 최초 항목을 기준으로 검사합니다."
            })
        else:
    ...
    for year, sname in targets.items():
        ...
        for rr in range(first_row, last_row + 1):
            ...
            if year == 2024:
                ...
                if is_bidirectional:
                    ...
                    if missing:
                        ...
                        issues.append({"severity": "ERROR", "sheet": sname, "row": rr, "message": f"↔ 과목명 중 목록에 없는 항목: {', '.join(missing)}{hint_txt}"})
            ...
            if hidden_rec is not None:
                ...
                if typ_s == "":
                    issues.append({"severity": "ERROR", "sheet": sname, "row": rr, "message": f"유형(C{rr})이 비어 있습니다. (목록: {hidden_rec['type']})"})
                elif typ_s != hidden_rec["type"]:
                    issues.append({"severity": "ERROR", "sheet": sname, "row": rr, "message": f"유형 불일치: 시트='{typ_s}' / 목록='{hidden_rec['type']}'"})
                ...
                if basic_n is None:
                    ...
                    if hidden_rec["basic"] is not None and abs(basic_n - hidden_rec["basic"]) > EPS:
                        issues.append({"severity": "ERROR", "sheet": sname, "row": rr, "message": f"기본학점 불일치: 시트={basic_n:g} / 목록={hidden_rec['basic']:g}"})

                grade_v, _, _ = get_value_with_merge(ws_v, ws_f, merge_lookup, rr, grading_col)
                grade_s = safe_strip(grade_v)
                if grade_s == "":
                    issues.append({"severity": "ERROR", "sheet": sname, "row": rr, "message": f"성적처리 유형(O{rr})이 비어 있습니다. (목록: {hidden_rec['grading']})"})
                elif grade_s != hidden_rec["grading"]:
                    # 과목명에 괄호가 있는 경우 CHECK로 처리
                    has_parenthesis = "(" in str(course_raw) or ")" in str(course_raw)
                    if has_parenthesis:
                        issues.append({"severity": "CHECK", "sheet": sname, "row": rr, "message": f"성적처리 유형을 확인하세요. 이상없으면 무시하셔도 됩니다. (시트='{grade_s}' / 목록='{hidden_rec['grading']}')"})
                    else:
                        issues.append({"severity": "ERROR", "sheet": sname, "row": rr, "message": f"성적처리 유형 불일치: 시트='{grade_s}' / 목록='{hidden_rec['grading']}'"})
                ...
                if abs(sem_sum) <= EPS:
                    ...
                    if abs(op_n - sem_sum) > EPS:
                        ...
                        if abs(sem_sum - op_n * 2) <= EPS:
                            issues.append({"severity": "CHECK", "sheet": sname, "row": rr, "message": f"운영학점({op_col_name}열)과 편성된 학점({sem_cols_name}열) 불일치: 운영학점={op_n:g}, {sem_cols_name}합={sem_sum:g} (학기제라면 오류가 아닙니다)"})
                        else:
                            issues.append({"severity": "ERROR", "sheet": sname, "row": rr, "message": f"운영학점({op_col_name}열)과 편성된 학점({sem_cols_name}열) 불일치: 운영학점={op_n:g}, {sem_cols_name}합={sem_sum:g}"})
            ...
            if rng.min_col in total_cols and rng.max_col == rng.min_col:
                ...
                if total_n is None:
                    ...
                    if abs(total_n - expected) > EPS:
                        issues.append({"severity": "ERROR", "sheet": sname, "row": rng.min_row, "message": f"{col_name}열 병합구간 합계 불일치: 셀값={total_n:g}, 기대값({sem_cols_name}합)={expected:g} (구간 {start}~{end}행, 색깔행 제외)"})
...
class App:
    ...
    def _print_summary(self, summary, issues):
        ...
        if hidden:
            self._w(tab, f"- 목록 시트: {hidden} (과목 {cnt}개)\n", "INFO")
        else:
            self._w(tab, "- 목록 시트: (없음)\n", "ERROR")
