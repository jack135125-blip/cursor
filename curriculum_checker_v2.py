...
def cell_has_color(cell) -> bool:
    """셀에 '눈에 보이는 채우기 색'이 있는지 판정(흰색/기본 채우기는 제외)."""
    try:
        fill = cell.fill
        if fill is None:
            return False
        ...
        ftype = getattr(fg, "type", None)
        if ftype == "rgb":
            rgb = (fg.rgb or "").upper()
            # openpyxl에서 기본/미지정은 00000000 또는 빈값으로 오는 경우가 있음
            if rgb in ("", "00000000", "FFFFFFFF"):
                return False
            return True

        # theme/indexed는 사용자가 색을 줬을 가능성이 높다고 보고 True 처리
        if ftype in ("theme", "indexed"):
            return True
...
def run_checks(xlsx_path: str):
    ...
    for year, sname in targets.items():
        ...
        for rr in range(first_row, last_row + 1):
            ...
            # (일부 행은 과목명이 있지만 학점 배치가 아직 안 되어 있을 수 있음)
        ...
        for rr in range(first_row, last_row + 1):
            ...
            if is_error_token(course_raw):
                continue  # row_total 계산 단계에서 이미 보고됨

            # 2025/2026: 과목명 셀(D열)에 채우기 색이 있는 경우는 과목명 일치 여부를 검사하지 않음
            # 2024: 과목명 일치 여부 자체를 검사하지 않음
            colored = False
            try:
                colored = cell_has_color(ws_f.cell(rr, course_col))
            except Exception:
                colored = False
            skip_name_check = (year == 2024) or ((year in (2025, 2026)) and colored)
            ...
            # 운영학점(F)과 G~L 합 비교는 과목명 일치 여부와 무관하게 수행
            ...
            if op_n is None:
                ...
                # (1) G~L 합이 0이면, 운영학점은 '바로 위(F{rr-1})'와 같으면 OK
                if abs(sem_sum) <= 1e-9:
                    above_ok = False
                    if rr > first_row:
                        above_v, _, _ = get_value_with_merge(ws_v, ws_f, merge_lookup, rr - 1, op_col)
                        above_n = to_number(above_v)
                        if above_n is not None and abs(op_n - above_n) <= 1e-9:
                            above_ok = True

                    if not above_ok and abs(op_n - sem_sum) > 1e-9:
                        if rr > first_row:
                            issues.append({"severity": "ERROR", "sheet": sname, "row": rr,
                                           "message": f"운영학점(F)과 G~L 합 불일치: 운영학점={op_n:g}, G~L합={sem_sum:g} (G~L합이 0이어서 위 셀 F{rr-1}과 비교했으나 불일치)"})
                        else:
                            issues.append({"severity": "ERROR", "sheet": sname, "row": rr,
                                           "message": f"운영학점(F)과 G~L 합 불일치: 운영학점={op_n:g}, G~L합={sem_sum:g}"})
                else:
                    if abs(op_n - sem_sum) > 1e-9:
                        issues.append({"severity": "ERROR", "sheet": sname, "row": rr,
                                       "message": f"운영학점(F)과 G~L 합 불일치: 운영학점={op_n:g}, G~L합={sem_sum:g}"})

            # (4) '음악↔미술' 형태면, 화살표 앞/뒤 과목명을 각각 숨김 시트에서 확인
            if "↔" in course_norm:
                parts = [p.strip() for p in course_norm.split("↔") if p.strip() != ""]
                if len(parts) >= 2 and not skip_name_check:
                    missing = [p for p in parts if p not in hidden]
                    if missing:
                        issues.append({"severity": "ERROR", "sheet": sname, "row": rr,
                                       "message": f"과목명(↔) 중 숨김 시트에 없는 항목: {', '.join(missing)} (원문: '{course_norm}')"})
                # ↔ 행은 단일 과목으로 매핑하기 어렵기 때문에 유형/학점/성적처리/범위 비교는 생략
                continue

            # 숨김 기준 레코드 조회
            h = hidden.get(course_norm)

            # 과목명 일치 여부 검사(요구사항: 2024는 생략, 2025/2026은 색 있는 셀 생략)
            if h is None:
                if not skip_name_check:
                    ...
                    issues.append({"severity": "ERROR", "sheet": sname, "row": rr,
                                   "message": f"숨김 시트 과목명과 불일치: '{course_norm}'{hint}"})
                # 매칭이 안 되면 이후(유형/기본학점/성적처리/범위)는 수행 불가
                continue
            ...
            # 운영학점 범위 체크(숨김 최소~최대) - 운영학점이 숫자인 경우만
            if op_n is not None and h["min"] is not None and h["max"] is not None:
        ...
        # (6) M/N 병합 구간 합계 체크
        # 병합 범위: ws_f.merged_cells (수식 시트 기준)
        # - 각 범위의 top-left만 검사
        checked_spans = set()
            ...
            if rng.min_col in total_cols and rng.max_col == rng.min_col:
                ...
                if total_n is None:
                    if total_formula:
                        ...
                        # 값이 비어 있으면 강제 오류로 보긴 애매하니 경고 처리
        ...
        # 병합이 아닌 단일 셀 합계도 존재할 수 있으므로(혹시 모를 케이스) top-left 기준 보조 체크
        for col in total_cols:
            for rr in range(first_row, last_row + 1):
                # 병합 셀의 top-left면 위에서 처리됨
                if (rr, col) in merge_lookup:
                    min_r, min_c, _, _ = merge_lookup[(rr, col)]
                    if not (rr == min_r and col == min_c):
                        continue
                    # top-left는 위 병합 루프에서 검사됨
                    continue
...
class App:
    def __init__(self, root: tk.Tk):
        ...
        self.style.configure("Accent.TButton", font=("Malgun Gothic", 11, "bold"), padding=(14, 8))
        self.style.map("Accent.TButton",
                       background=[("active", self.colors["accent"]), ("!active", self.colors["accent"])],
                       foreground=[("active", "white"), ("!active", "white")])

        # 일부 ttk 테마는 Button 배경을 잘 안 받으므로, 안전하게 일반 버튼도 함께 제공
        self._build_ui()
